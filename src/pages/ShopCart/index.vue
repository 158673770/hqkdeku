      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cart.skuNum" minnum="1" @change="handler('change',$event.target.value*1,cart)" class="itxt">
          ...
          <li class="cart-list-con7">
            <a href="#none" class="sindelet" @click="deletCartById(cart)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
import throttle from 'lodash/throttle'
  export default {
    ...
    methods:{
      ...
      // 修改某个产品参数
      // async 与 await 配合 try 与 catch 使用
      handler:throttle( async function(type,disNum,cart){
          switch(type){
            ...
            case "minus":
              ...
              //   disNum = 0
              // }
              disNum = cart.skuNum > 1 ? -1 : 0  //上面if判断的简写三元运算
              break;
            case 'change':
          ...
          // 派发action给服务器发请求
          // console.log(type,"disNum",disNum,cart)
          try{
            // 代表的是修改成功
     ...
     async UpDateChecked(cart,event){
          ...
          try{
            await this.$store.dispatch("UpDateCheckCart",{skuId:cart.skuId,isChecked})
            this.getData()
          }catch(error){
            alert(error.message)
          }
    ...
    computed:{
      ...
      isAllCheck(){
        // every 遍历每个元素判断字段中的某个值是否为真 只要全等于返回真 如果有一个不通就返回false
